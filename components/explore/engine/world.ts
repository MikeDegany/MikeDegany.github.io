...
const ROOM_SPECS: RoomSpec[] = [
  ...
  { name: "EDUCATION", height: 1000, door: { x: 400 }, floor: "#17335f" },
  { name: "TECHNICAL SKILLS", height: 1100, door: { x: 700 }, floor: "#1d2e58" },
  { name: "PROJECTS", height: 1500, door: { x: 400 }, floor: "#183462" },
...
export function buildWorld(): World {
  ...
  // ─── SKILLS (scattered, slightly askew containers) ──────────────────────────
  {
    ...
    // into view as you pass. Gaps stay ≥ 130 so the planner (robot-radius inflated) can thread
    // them; entry lane (x 400–600, bottom) and exit band (x 700–900, top) stay clear.
    const spots: [number, number, number][] = [
      [150, 110, -0.1], [500, 95, 0.12],
      [150, 280, 0.14], [500, 270, -0.18], [850, 260, 0.08],
      [300, 470, -0.07], [600, 480, 0.2], [900, 470, -0.12],
      [140, 660, 0.16], [470, 690, -0.05], [800, 670, 0.11],
      [280, 870, 0.06], [720, 880, -0.16],
    ]
    skills.forEach((sk, k) => {
      const [cx, cy, ang] = spots[k]
      const TW = widthFor(sk.name)
      const y = r.y0 + cy
      const crate: Crate = { skillId: sk.id, name: sk.name, color: sk.color, x: cx - TW / 2, y: y - TH / 2, w: TW, h: TH }
      const idx = crates.push(crate) - 1
      segments.push(...rotRectSegments(cx, y, TW, TH, ang, i, idx))
      decor.push({ kind: "rect", x: cx - TW / 2, y: y - TH / 2, w: TW, h: TH, fill: sk.color, stroke: "rgba(255,255,255,0.55)", radius: 8, angle: ang })
      decor.push({ kind: "text", text: sk.name, x: cx, y: y + 1, size: 18, color: "#0b1120", weight: 800, align: "center", maxWidth: TW - 14, maxLines: 1, angle: ang, ghost: true, ghostColor: sk.color })
    })
    beacons.push({
      id: "skills",
      kind: "skills",
  ...
  // vertices, graph nodes, arc-length station ticks) ──────────────────────────
  {
    ...
    const BOUNDARY = "#4ade80" // road boundary (green)
    const DIVIDER = "#fb923c" // lane centerline (orange, dashed)
    const NODE_COLOR = "#60a5fa" // graph node / label accent (blue)
    ...
    const AMP = 70
    const stationY = (k: number) => bottom - 200 - (k * (bottom - top - 400)) / (n - 1)
    ...
    for (const off of [-HALF, 0, HALF]) {
      for (let idx = 0; idx < waypoints.length - 1; idx++) {
        ...
        for (let s = 0; s <= steps; s++) {
          const t = s / steps
          decor.push({ kind: "circle", x: a.x + (b.x - a.x) * t + off, y: a.y + (b.y - a.y) * t, r: off === 0 ? 1.2 : 1.5, fill: off === 0 ? DIVIDER : BOUNDARY })
        }
      }
    }
    ...
    decor.push({ kind: "text", text: "HD MAP · VECTOR LANE GRAPH", x: 60, y: r.y0 + 60, size: 13, color: "rgba(226,232,240,0.7)", weight: 700, letterSpacing: 4 })
    decor.push({ kind: "path", points: [{ x: 60, y: r.y0 + 88 }, { x: 82, y: r.y0 + 88 }], stroke: BOUNDARY, width: 1.5 })
    decor.push({ kind: "text", text: "boundary", x: 90, y: r.y0 + 88, size: 12, color: "rgba(226,232,240,0.75)", weight: 600 })
    decor.push({ kind: "path", points: [{ x: 178, y: r.y0 + 88 }, { x: 200, y: r.y0 + 88 }], stroke: DIVIDER, width: 1.5, dash: [6, 5] })
    decor.push({ kind: "text", text: "centerline", x: 208, y: r.y0 + 88, size: 12, color: "rgba(226,232,240,0.75)", weight: 600 })
    decor.push({ kind: "circle", x: 306, y: r.y0 + 88, r: 3, fill: NODE_COLOR })
    decor.push({ kind: "text", text: "node", x: 316, y: r.y0 + 88, size: 12, color: "rgba(226,232,240,0.75)", weight: 600 })

    // Scale bar near the entrance for cartographic authenticity.
    const SCALE = 0.15
    const scaleLen = 100
    const sbY = bottom - 20
    decor.push({ kind: "path", points: [{ x: 60, y: sbY }, { x: 60 + scaleLen, y: sbY }], stroke: "rgba(226,232,240,0.6)", width: 2 })
    decor.push({ kind: "path", points: [{ x: 60, y: sbY - 6 }, { x: 60, y: sbY + 6 }], stroke: "rgba(226,232,240,0.6)", width: 2 })
    decor.push({ kind: "path", points: [{ x: 60 + scaleLen, y: sbY - 6 }, { x: 60 + scaleLen, y: sbY + 6 }], stroke: "rgba(226,232,240,0.6)", width: 2 })
    decor.push({ kind: "text", text: `≈ ${Math.round(scaleLen * SCALE)} m`, x: 60 + scaleLen / 2, y: sbY - 16, size: 11, color: "rgba(226,232,240,0.6)", align: "center", weight: 600 })
